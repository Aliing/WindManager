...
public class WinDES {
	...
	final static byte perm4[] = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9,
			10, 11, 12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20,
			21, 22, 23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1 };

	final static byte perm5[] = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26,
			5, 18, 31, 10, 2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11,
			4, 25 };

	final static byte perm6[] = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15,
	...
	private static void concat(byte out[], byte in1[], byte in2[], int l1,
			int l2) {
		int di = 0;
		for (int i = 0; i < l1; i++) {
			out[di++] = in1[i];
	...
	private static void dohash(byte out[], byte in[], byte key[], boolean forw) {
		...
		for (i = 0; i < 16; i++) {
			...
			for (j = 0; j < 8; j++) {
				...
				for (k = 0; k < 4; k++) {
					b[j][k] = ((sbox[j][m][n] & (1 << (3 - k))) != 0) ? (byte) 1
							: (byte) 0;
				}
			}
	...
	private static void smbhash(byte out[], byte in[], byte key[], boolean forw) {
		...
		for (i = 0; i < 64; i++) {
			inb[i] = ((in[i / 8] & (1 << (7 - (i % 8)))) != 0) ? (byte) 1
					: (byte) 0;
			keyb[i] = ((key2[i / 8] & (1 << (7 - (i % 8)))) != 0) ? (byte) 1
					: (byte) 0;
			outb[i] = 0;
		}
	...
	/*
	 * void E_P24(const uchar *p21, const uchar *c8, uchar *p24) { smbhash(p24,
	 * c8, p21, 1); smbhash(p24+8, c8, p21+7, 1); smbhash(p24+16, c8, p21+14,
	 * 1); }
	 * 
	 * void D_P16(const uchar *p14, const uchar *in, uchar *out) { smbhash(out,
	 * in, p14, 0); smbhash(out+8, in+8, p14+7, 0); }
	 * 
	 * void E_old_pw_hash( const uchar *p14, const uchar *in, uchar *out) {
	 * smbhash(out, in, p14, 1); smbhash(out+8, in+8, p14+7, 1); }
	 * 
	 * void cred_hash1(uchar *out, const uchar *in, const uchar *key) { uchar
	 * buf[8];
	 * 
	 * smbhash(buf, in, key, 1); smbhash(out, buf, key+9, 1); }
	 * 
	 * void cred_hash2(uchar *out,uchar *in,uchar *key) { uchar buf[8]; static
	 * uchar key2[8];
	 * 
	 * smbhash(buf, in, key, 1); key2[0] = key[7]; smbhash(out, buf, key2, 1); }
	 * 
	 * void cred_hash3(uchar *out, const uchar *in,uchar *key, int forw) {
	 * static uchar key2[8];
	 * 
	 * smbhash(out, in, key, forw); key2[0] = key[7]; smbhash(out + 8, in + 8,
	 * key2, forw); }
	 * 
	 * void SamOEMhash( uchar *data, const uchar *key, int val) { uchar
	 * hash[256]; uchar index_i = 0; uchar index_j = 0; uchar j = 0; int ind;
	 * int len = 0; if (val == 1) len = 516; if (val == 0) len = 16; if (val ==
	 * 3) len = 8; if (val == 2) len = 68; if (val == 4) len = 32;
	 * 
	 * for (ind = 0; ind < 256; ind++) { hash[ind] = (uchar)ind; }
	 * 
	 * for( ind = 0; ind < 256; ind++) { uchar tc;
	 * 
	 * j += (hash[ind] + key[ind%16]);
	 * 
	 * tc = hash[ind]; hash[ind] = hash[j]; hash[j] = tc; } for( ind = 0; ind <
	 * len; ind++) { uchar tc; uchar t;
	 * 
	 * index_i++; index_j += hash[index_i];
	 * 
	 * tc = hash[index_i]; hash[index_i] = hash[index_j]; hash[index_j] = tc;
	 * 
	 * t = hash[index_i] + hash[index_j]; data[ind] = data[ind] ^ hash[t]; } }
	 * 
	 * void sam_pwd_hash(uint32 rid, const uchar *in, uchar *out, int forw) {
	 * uchar s[14];
	 * 
	 * s[0] = s[4] = s[8] = s[12] = (uchar)(rid & 0xFF); s[1] = s[5] = s[9] =
	 * s[13] = (uchar)((rid >> 8) & 0xFF); s[2] = s[6] = s[10] = (uchar)((rid >>
	 * 16) & 0xFF); s[3] = s[7] = s[11] = (uchar)((rid >> 24) & 0xFF);
	 * 
	 * smbhash(out, in, s, forw); smbhash(out+8, in+8, s+7, forw); }
	 */
