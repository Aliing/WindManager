/*
  ...
  *
  */
package com.ericdaugherty.soht.client.util;
...
public class AvailablePortFinder {
	 ...
	 * @return a {@link Set} of currently available port numbers
	 */
...
//		 throw new NoSuchElementException("Could not find an available port above " + fromPort);
		 ...
		 return getNextAvailable(fromPort, MAX_PORT_NUMBER);
	 }

     /**
      * Gets the next available port starting at a port.
      *
      * @param fromPort the port start to scan for availability
	  * @param toPort the port end to scan for availability
	  * @return the next available port number
      * @throws NoSuchElementException if there are no ports available
      */
	public static int getNextAvailable(int fromPort, int toPort) {
		 ...
		 if (toPort > MAX_PORT_NUMBER) {
			 throw new IllegalArgumentException("Invalid end port: " + toPort);
		 }

		 if (fromPort > toPort) {
			 throw new IllegalArgumentException("Start port " + fromPort + " must not be larger than end port " + toPort);
		 }
		 ...
		 throw new NoSuchElementException("Could not find an available port above " + fromPort + " as well as below " + toPort);
	 }
	 ...
	 * @return true if the given port is available, false otherwise
	 */
	public static boolean available(int port) {
     ...
     /**
      * Returns the {@link Set} of currently available port numbers ({@link Integer})
      * between the specified port range.
      *
	  * @param fromPort the port start to scan
	  * @param toPort the port end to scan
	  * @return a {@link Set} of currently available port numbers
