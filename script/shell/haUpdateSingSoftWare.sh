...
##remove domains and res in hm_soft_upgrade before cp
rm -rf $UPDATE_HOME/hm/domains
rm -rf $UPDATE_HOME/hm/res
...
then
  ...
  /bin/mv -f $UPDATE_HOME/HiveManager/cas  $CATALINA_HOME/webapps
               SHELL_FILE_NAMES=`/bin/ls $CATALINA_HOME/webapps/cas/ |grep .sh`
		       if [ ! "$SHELL_FILE_NAMES" = "" ]
		       then
