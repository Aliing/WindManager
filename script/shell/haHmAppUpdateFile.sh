...
echo "haHmAppUpdateFiles.sh start..." >>$HM_SCRIPT_LOGFILE
...
then
  ...
  if [ -f /root/.auto_jvm_memory ]; then
  	/bin/cp -f /root/.auto_jvm_memory /hivemap/root >>$HM_SCRIPT_LOGFILE 2>&1
  else
    /bin/rm -f /hivemap/root/.auto_jvm_memory >>$HM_SCRIPT_LOGFILE 2>&1
  fi
	_OLD_FILE=$HM_ROOT/WEB-INF/hmconf/jvmoption.conf.default
	_NEW_FILE=$UPDATE_HOME/hm/WEB-INF/hmconf/jvmoption.conf.default
	OLD_JVM_MEMORY=`sed -n "s/MAX_MEMORY=\(.*\)/\1/p" $_OLD_FILE 2>/dev/null`
	if [ -n "$OLD_JVM_MEMORY" ]; then
		sed -i "s/MAX_MEMORY=.*/MAX_MEMORY=${OLD_JVM_MEMORY}/" $_NEW_FILE
	fi
...
echo "change db conn of image and capwap..." >> $HM_SCRIPT_LOGFILE 2>&1
...
##copy pgpool file
if [ -f /HiveManager/PGPool/script/pgpool/clone.conf ]
then
	/bin/cp -f /HiveManager/PGPool/script/pgpool/clone.conf /hivemap/HiveManager/PGPool/script/pgpool
else
    /bin/rm -f /hivemap/HiveManager/PGPool/script/pgpool/clone.conf
fi
if [ -f /HiveManager/PGPool/script/base/customconf.sh ]
then
	/bin/cp -f /HiveManager/PGPool/script/base/customconf.sh /hivemap/HiveManager/PGPool/script/base/
	chmod u+x /hivemap/HiveManager/PGPool/script/base/customconf.sh
else
   /bin/rm -f /hivemap/HiveManager/PGPool/script/base/customconf.sh
fi
...
##master_ip=`cat /HiveManager/tomcat/hm_soft_upgrade/appserver | grep master | sed -n "s/^master\:([0-9\.]*\)/\1/p"`
...
##rm -rf /HiveManager/tomcat/hm_soft_upgrade/dbserver

echo "haHmAppUpdateFiles end." >>$HM_SCRIPT_LOGFILE
