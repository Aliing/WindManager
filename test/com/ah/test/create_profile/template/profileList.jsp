		...
		<tr>
			...
			<table cellspacing="0" cellpadding="0" border="0" class="view">
				<tr>
					...
					<th align="left" nowrap><ah:sort name="profileName"
				...
				<s:iterator value="page" status="status">
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						<s:if test="%{showDomain && owner.domainName!='home' && owner.domainName!='global'}">
   						...
   						<s:if test="%{showDomain}">
							<td class="list"><a
								href='<s:url value="ipAddress.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{id}"/><s:param name="domainId" value="%{owner.id}"/></s:url>'><s:property
								value="profileName" /></a></td>
						</s:if>
						<s:else>
							<td class="list"><a
								href='<s:url value="ipAddress.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{id}"/></s:url>'><s:property
								value="profileName" /></a></td>
						</s:else>
