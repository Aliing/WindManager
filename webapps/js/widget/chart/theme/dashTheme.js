(function($){
	...
	var dashTheme = function(chart, options) {
		...
		var refreshArea = chart.container + '_tip_update_area';
		chart.AREA_SECTIONS.TIP_UPDATE_AREA = {};
		chart.AREA_SECTIONS.TIP_UPDATE_AREA.CONTAINER = refreshArea;
		chart.AREA_SECTIONS.TIP_UPDATE_AREA.REFRESH_CTL = refreshArea + 'refresh_ctl';
		chart.AREA_SECTIONS.TIP_UPDATE_AREA.REFRESH_INFO = refreshArea + 'refresh_tip';
		...
		var $div1TitleCtl = $("<div></div>")
							...
							.addClass('rpTitleCtlStyleTitleDash');
	...
	Chart.prototype.__onStatusChanged = function(oldStatus, status) {
		...
		if (this.STATUS_MODE.EDIT === status) {
			$el(this.AREA_SECTIONS.TIP_UPDATE_AREA.CONTAINER).hide();
			$el(this.AREA_SECTIONS.SUB_TITLE_CONTAINER).hide();
		} else {
			$el(this.AREA_SECTIONS.TIP_UPDATE_AREA.CONTAINER).show();
			$el(this.AREA_SECTIONS.SUB_TITLE_CONTAINER).show();
		}
	};
