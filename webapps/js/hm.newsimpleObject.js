...
hm.simpleObject.createMacOverlay = function(destination, type){
	...
	var overlay = new YAHOO.widget.Panel(destination + "_mac", { width:"280px", visible:false, draggable:false, close:true, underlay:"none" } );
...
hm.simpleObject.createIpOverlay = function(destination, type){
	...
	// Instantiate a Panel from script 
	var overlay = new YAHOO.widget.Panel(destination + "_ip", { width:"280px", visible:false, draggable:false, close:true, underlay:"none" } );
	overlay.setHeader(header);
...
hm.simpleObject.createVlanOverlay = function(destination){
	// Instantiate a Panel from script 
	var overlay = new YAHOO.widget.Panel(destination + "_vlan", { width:"280px", visible:false, draggable:false, close:true, underlay:"none" } );
...
hm.simpleObject.defaultSimpleProcess = function(o){
	...
	if(type && o.argument){
		if(type == "add"){ // add process
			...
			if(data.item){
				...
				hm.simpleObject.addOption(destEl, data.item.key, data.item.value, false);
			}
	...
	if(! data.msg){
		if(document.forms["qosClassification"] && document.forms["qosClassification"] != "undefined"){
			document.forms["qosClassification"].refreshPageFlag.value = "1";
			document.forms["qosClassification"].operation.value = "update";
			document.forms["qosClassification"].submit();
		}else if(document.forms["ipPolicy"] && document.forms["ipPolicy"] != "undefined"){
			document.forms["ipPolicy"].refreshPageFlag.value = "1";
			document.forms["ipPolicy"].operation.value = "update";
			document.forms["ipPolicy"].submit();
		}
	}
}
