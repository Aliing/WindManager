...
function createDeviceInventoryMenu() {
	...
	di_Items.push([{text: '<s:text name="hm.missionux.importserial.importserial"/>'}]);
	...
	di_Menu.subscribe('click', function (p_sType, p_aArguments){
		...
		if (menuText=='<s:text name="geneva_08.hm.menu.addImport"/>') {
			...
			submitAction('export')
		} else if (menuText=='<s:text name="hm.missionux.importserial.importserial"/>') {
			submitAction('importOPDevice')
		}
	});
...
function clickModifyButton() {
	hm.util.checkMultiModifyUnmanagedDevice('selectedIds',null,diid_apid_config_map, false);
}
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th align="left" nowrap><ah:sort name="serialNumber"
								key="hiveAp.serialNumber" /></th>
						</s:if>
						<s:elseif test="%{columnId == 2}">
							<th align="left" nowrap><ah:sort name="macaddress"
									key="hiveAp.macaddress" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 3}">
							<th align="left" nowrap><ah:sort name="hostname"
									key="hiveAp.hostName" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 4}">
							<th align="left" nowrap>
								<s:text name="geneva_08.device.inventory.connect.status" />
							</th>
						</s:elseif>
						<s:elseif test="%{columnId == 5}">
							<th align="left" nowrap>
								<ah:sort name="LOWER(configTemplate.configName)"
						...
						<s:elseif test="%{columnId == 7}">
							<th align="left" nowrap>
								<ah:sort name="mapContainer" key="geneva_08.device.inventory.list.title.topology" />
							</th>
						</s:elseif>
						<s:elseif test="%{columnId == 8}">
							<th align="left" nowrap>
								<ah:sort name="hiveapmodel"
									key="hiveAp.model" />
							</th>
						</s:elseif>
						<s:elseif test="%{columnId == 9}">
							<th align="left" nowrap>
								<ah:sort name="devicetype"
									key="hiveAp.device.type" />
							</th>
						</s:elseif>
						<s:elseif test="%{columnId == 10}">
							<th align="left" nowrap>
								<ah:sort name="cfgIpAddress"
   						...
   						<s:iterator value="%{selectedColumns}">
							...
							<s:elseif test="%{columnId == 4}">
								<td class="list" nowrap="nowrap">
									<s:if test="%{connected}">
										<div class='device-conn-status conn-status4'
											title='<s:property value="deviceStatusHMConnectedStr" />'></div>
									</s:if>
									<s:else>
										<div class='device-conn-status conn-status3'
											title='<s:property value="deviceStatusHMNotConnectedStr" />'></div>
									</s:else>
								</td>
							</s:elseif>
							<s:elseif test="%{columnId == 5}">
								<td class="list" nowrap="nowrap">
									...
									<s:else>
										<s:property value="configTemplateName" />&nbsp;
									</s:else>
								</td>
							</s:elseif>
							<s:elseif test="%{columnId == 6}">
							...
							<s:elseif test="%{columnId == 7}">
								<td class="list" nowrap="nowrap"><s:property value="topologyName" />&nbsp;</td>
							</s:elseif>
							<s:elseif test="%{columnId == 8}">
								<td class="list" nowrap="nowrap"><s:property value="hiveApModelString" />&nbsp;</td>
							</s:elseif>
							<s:elseif test="%{columnId == 9}">
								<td class="list" nowrap="nowrap"><s:property value="deviceCategory" />&nbsp;</td>
							</s:elseif>
							<s:elseif test="%{columnId == 10}">
