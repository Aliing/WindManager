...
<div id="content"><s:form action="mgmtServiceSnmp">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>" 
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('mgmtServiceSnmp')"
						<s:property value="writeDisabled" />></td>
		...
		<tr>
			...
			<table cellspacing="0" cellpadding="0" border="0" class="view">
				<tr>
					...
					<th align="left" nowrap><ah:sort name="contact" key="config.mgmtservice.contact" /></th>				
					</s:if>
					<s:if test="%{columnId == 3}">
					<th align="left" nowrap><ah:sort name="enableSnmp" key="config.mgmtservice.enable.snmp" /></th>
					</s:if>
					<s:if test="%{columnId == 4}">
					<th align="left" nowrap><ah:sort name="enableCapwap" key="config.mgmtservice.enable.capwap" /></th>		
					</s:if>
					<s:if test="%{columnId == 5}">
					<th align="left" nowrap><s:text name="config.mgmtservice.ip.addresses" /></th>		
					</s:if>
				...
				<s:iterator value="page" status="status" id="pageRow">
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:if test="%{columnId == 1}">
						<s:if test="%{showDomain}">
       						<td class="list"><a href='<s:url action="mgmtServiceSnmp"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/>
       							<s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property value="mgmtName" /></a></td>
    					</s:if>
    					<s:else>
							<td class="list"><a
								href='<s:url action="mgmtServiceSnmp"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
								value="mgmtName" /></a></td>
						</s:else>
						</s:if>
						<s:if test="%{columnId == 2}">
						<td class="list"><s:property value="contact" />&nbsp;</td>
						</s:if>
						<s:if test="%{columnId == 3}">
						<td class="list"><s:property value="enableSnmpValue" /></td>
						</s:if>
						<s:if test="%{columnId == 4}">
						<td class="list"><s:property value="enableCapwapValue" /></td>
						</s:if>
						<s:if test="%{columnId == 5}">
