...
<div id="content"><s:form action="locationServer">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>" 
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('locationServer')"
						<s:property value="writeDisabled" />></td>
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th><ah:sort name="name" key="config.ipFilter.name" /></th>
						</s:if>
						<s:elseif test="%{columnId == 2}">
							<th><ah:sort name="enableServer" key="config.location.server.enabled" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 3}">
							<th><ah:sort name="serviceType" key="config.location.server.serviceType" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 4}">
							<th><s:text name="config.radiusAssign.serverName" /></th>
						</s:elseif>
						...
						<s:if test="%{showDomain && owner.domainName!='home' && owner.domainName!='global'}">
   						...
   						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
		   						<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="locationServer.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="name" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="locationServer.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="name" /></a></td>
								</s:else>
							</s:if>
							<s:elseif test="%{columnId == 2}">
								<td class="list">
									<s:property value="enableServer" />
								</td>
							</s:elseif>
							<s:elseif test="%{columnId == 3}">
								<td class="list">
									<s:property value="serviceType4Show" />
								</td>
							</s:elseif>
							<s:elseif test="%{columnId == 4}">
								<td class="list">
									<s:property value="serverIpName4Show" />&nbsp;
								</td>
							</s:elseif>
							<s:elseif test="%{columnId == 5}">
