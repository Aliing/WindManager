...
<div id="content"><s:form action="mgmtServiceDns">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>" 
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('mgmtServiceDns')"
						<s:property value="writeDisabled" />></td>
		...
		<tr>
			...
			<table cellspacing="0" cellpadding="0" border="0"
				...
				<tr>
					...
					<th align="left" nowrap><ah:sort name="domainName" key="config.mgmtservice.domain.name" /></th>
					</s:if>
				...
				<s:iterator value="page" status="status" id="pageRow">
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:if test="%{columnId == 1}">
						<s:if test="%{showDomain}">
       						<td class="list"><a href='<s:url action="mgmtServiceDns"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/>
       							<s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property value="mgmtName" /></a></td>
    					</s:if>
    					<s:else>
							<td class="list"><a
								href='<s:url action="mgmtServiceDns"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
								value="mgmtName" /></a></td>
						</s:else>
						</s:if>
						<s:if test="%{columnId == 2}">
						...
						<s:if test="%{columnId == 3}">
						<td class="list"><s:select list="%{ipAddressValue}" cssStyle="width: 200px;"/></td>
						</s:if>
						<s:if test="%{columnId == 4}">
