...
<div id="content"><s:form action="pppoe">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>"
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('pppoe')"
						<s:property value="writeDisabled" />></td>
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th><ah:sort name="pppoeName" key="config.pppoe.name" /></th>
						</s:if>
						<s:elseif test="%{columnId == 2}">
							<th><ah:sort name="username" key="config.pppoe.username" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 3}">
							<th><s:text name="config.pppoe.password" /></th>
						</s:elseif>
						<!---<s:elseif test="%{columnId == 4}">
							<th><ah:sort name="domain" key="config.pppoe.domain" /></th>
						</s:elseif>-->
						<s:elseif test="%{columnId == 5}">
							<th><ah:sort name="encryptionMethod" key="config.pppoe.encryption.method" /></th>
						</s:elseif>
						...
						<s:if test="%{showDomain && owner.domainName!='home' && owner.domainName!='global'}">
   						...
   						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
		   						<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="pppoe.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="pppoeName" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="pppoe.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="pppoeName" /></a></td>
								</s:else>
							</s:if>
							<s:elseif test="%{columnId == 2}">
							...
							<s:elseif test="%{columnId == 3}">
								<td class="list"><s:property value="strPasswordString" /></td>
							</s:elseif>
							<!--<s:elseif test="%{columnId == 4}">
								<td class="list"><s:property value="domain" /></td>
							</s:elseif>-->
							<s:elseif test="%{columnId == 5}">
								<td class="list"><s:property value="strEncryptionMethod" /></td>
							</s:elseif>
							<s:elseif test="%{columnId == 6}">
