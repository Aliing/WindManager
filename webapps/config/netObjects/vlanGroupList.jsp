...
<div id="content"><s:form action="vlanGroup">
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th align="left" nowrap><ah:sort name="vlanGroupName"
								key="config.vlanGroup.name" /></th>
						</s:if>
						<s:elseif test="%{columnId == 2}">
							<th align="left" nowrap>
								<s:text name="config.ipPolicy.description"/></th>
						</s:elseif>
						<s:elseif test="%{columnId == 3}">
							<th align="left" nowrap>
								<s:text name="config.vlanGroup.vlans"/></th>
						</s:elseif>
					</s:iterator>
						...
						<s:if test="%{showDomain && owner.domainName!='home' && owner.domainName!='global'}">
   						...
   						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
		   						<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="vlanGroup.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="vlanGroupName" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="vlanGroup.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="vlanGroupName" /></a></td>
								</s:else>
							</s:if>
							<s:elseif test="%{columnId == 2}">
							...
							<s:elseif test="%{columnId == 3}">
								<td class="list"><s:property value="vlans" /></td>
							</s:elseif>
						</s:iterator>
						<s:if test="%{showDomain}">
