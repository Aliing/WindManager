...
<div id="content"><s:form action="macFilters">
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th align="left" nowrap><ah:sort name="filterName" key="config.macFilter.name" /></th>
						</s:if>
				...
				<s:iterator value="page" status="status" id="pageRow">
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
								<s:if test="%{showDomain}">
		       						<td class="list"><a href='<s:url action="macFilters"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/>
		       							<s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property value="filterName" /></a></td>
		    					</s:if>
		    					<s:else>
									<td class="list"><a	href='<s:url action="macFilters"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
