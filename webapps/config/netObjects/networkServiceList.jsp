...
<div id="content"><s:form action="networkService">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>"
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('networkService')"
						<s:property value="writeDisabled" />></td>
					<%--
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th><ah:sort name="serviceName" key="config.ns.name" /></th>
						</s:if>
						<s:elseif test="%{columnId == 2}">
							<th><ah:sort name="protocolId" key="config.ns.protocolNumber" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 3}">
							<th><ah:sort name="portNumber" key="config.ns.portNumber" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 4}">
							<th><ah:sort name="idleTimeout" key="config.ns.idleTimeout" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 5}">
							<th><ah:sort name="algType" key="config.ns.algTypename" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 6}">
							<th><ah:sort name="description" key="config.ns.description" /></th>
						</s:elseif>
					</s:iterator>
				...
				<s:iterator value="page" status="status" id="pageRow">
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						<s:if test="%{showDomain && owner.domainName!='home' && owner.domainName!='global'}">
   						...
   						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
								<td class="list"><s:if test="%{defaultFlag}">
									<s:property value="serviceName" />
								</s:if><s:else>
									<s:if test="%{showDomain}">
										<a href='<s:url value="networkService.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
											value="serviceName" /></a>
									</s:if>
									<s:else>
										<a href='<s:url value="networkService.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
											value="serviceName" /></a>
									</s:else>
								</s:else></td>
							</s:if>
							<s:elseif test="%{columnId == 2}">
								<td class="list"><s:property value="protocolStr" /></td>
							</s:elseif>
							<s:elseif test="%{columnId == 3}">
								<td class="list"><s:property value="portNumberString" /></td>
							</s:elseif>
							<s:elseif test="%{columnId == 4}">
								<td class="list"><s:property value="idleTimeout" /></td>
							</s:elseif>
							<s:elseif test="%{columnId == 5}">
								<td class="list">
									<s:if test="%{'' == algTypeStr}">&nbsp;</s:if>
									<s:else><s:property value="algTypeStr" /></s:else>
								</td>
							</s:elseif>
							<s:elseif test="%{columnId == 6}">
								<td class="list">
									<s:if test="%{'' == description}">&nbsp;</s:if>
