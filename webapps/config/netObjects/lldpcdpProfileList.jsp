...
<script
	src="<s:url value="/js/hm.paintbrush.js" includeParams="none"/>?v=<s:property value="verParam" />"></script>
...
<div id="content"><s:form action="lldpcdpProfiles">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore"
						value="<s:text name="button.paintbrush"/>" id="brushTriggerBtn"
						class="button"
						onclick="hm.paintbrush.triggerPaintbrush('lldpcdpProfiles')"
						<s:property value="writeDisabled" />></td>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th><ah:sort name="profileName"
								key="config.lldpcdpprofile.name" /></th>
						</s:if>
						<s:if test="%{columnId == 2}">
							<th><ah:sort name="description"
								key="config.ipFilter.description" /></th>
						</s:if>
					</s:iterator>
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
								<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="lldpcdpProfiles.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="profileName" /> </a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="lldpcdpProfiles.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="profileName" /> </a></td>
								</s:else>
							</s:if>
							<s:if test="%{columnId == 2}">
								<td class="list"><s:property value="description" /> &nbsp;
								</td>
							</s:if>
						</s:iterator>
						<s:if test="%{showDomain}">
