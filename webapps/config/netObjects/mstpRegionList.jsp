...
<div id="content"><s:form action="mstpRegion">
		...
		<tr>
			...
			<table cellspacing="0" cellpadding="0" border="0" class="view">
				<tr>
					...
					<th align="left" nowrap><ah:sort name="regionName" key="config.switchSettings.mstp.name" /></th>
					</s:if>
					<s:if test="%{columnId == 2}">
					<th align="left" nowrap><ah:sort name="description" key="config.switchSettings.mstp.description" /></th>
					</s:if>
					<s:if test="%{columnId == 3}">
					<th align="left" nowrap><ah:sort name="revison" key="config.switchSettings.mstp.revision" /></th>
					</s:if>
					<s:if test="%{columnId == 4}">
					<th align="left" nowrap><ah:sort name="hops" key="config.switchSettings.mstp.maxhops" /></th>
					</s:if>
					</s:iterator>
				...
				<s:iterator value="page" status="status" id="pageRow">
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:if test="%{columnId == 1}">
						<s:if test="%{showDomain}">
       						<td class="list"><a href='<s:url action="mstpRegion"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/>
       							<s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property value="regionName" /></a></td>
    					</s:if>
    					<s:else>
							<td class="list"><a
								href='<s:url action="mstpRegion"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
								value="regionName" /></a></td>
						</s:else>
						</s:if>
						<s:if test="%{columnId == 2}">
						...
						<s:if test="%{columnId == 3}">
						<td class="list"><s:property value="revision" /></td>
						</s:if>
						<s:if test="%{columnId == 4}">
						<td class="list"><s:property value="hops" /></td>
						</s:if>
						</s:iterator>
