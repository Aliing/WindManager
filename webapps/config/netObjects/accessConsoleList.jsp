...
<div id="content"><s:form action="accessConsole">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>"
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('accessConsole')"
						<s:property value="writeDisabled" />></td>
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th><ah:sort name="consoleName" key="config.ipFilter.name" /></th>
						</s:if>
						<s:elseif test="%{columnId == 2}">
							<th><ah:sort name="consoleMode" key="config.access.console.mode" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 3}">
							<th><ah:sort name="maxClient" key="config.ssid.maxClient" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 4}">
							<th><ah:sort name="mgmtKey" key="config.ssid.keyManagement" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 5}">
							<th><ah:sort name="encryption" key="config.ssid.encriptionMethord" /></th>
						</s:elseif>
						...
						<s:if test="%{showDomain && owner.domainName!='home' && owner.domainName!='global'}">
   						...
   						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
		   						<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="accessConsole.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="consoleName" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="accessConsole.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="consoleName" /></a></td>
								</s:else>
							</s:if>
							<s:elseif test="%{columnId == 2}">
								<td class="list"><s:property value="strConsoleMode" /></td>
							</s:elseif>
							<s:elseif test="%{columnId == 3}">
								<td class="list"><s:property value="maxClient" /></td>
							</s:elseif>
							<s:elseif test="%{columnId == 4}">
								<td class="list"><s:property value="strMgmtKey" /></td>
							</s:elseif>
							<s:elseif test="%{columnId == 5}">
								<td class="list"><s:property value="strEncryption" /></td>
							</s:elseif>
							<s:elseif test="%{columnId == 6}">
