			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>"
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('configmdm')"
						<s:property value="writeDisabled" />></td>
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th align="left" nowrap><ah:sort name="policyname" key="config.ssid.advanced.mdm.enrollment.name" /></th>
						</s:if>
						<s:if test="%{columnId == 2}">
							<th align="left" nowrap><ah:sort name="description" key="config.ssid.advanced.mdm.enrollment.description" /></th>
						</s:if>
						<s:if test="%{columnId == 3}">
							<th align="left" nowrap><ah:sort name="mdmtype" key="config.ssid.advanced.mdm.enrollment.type" /></th>
						</s:if>
						<s:if test="%{columnId == 4}">
							<th align="left" nowrap><s:text	name="config.ssid.advanced.mdm.enrollment.osobj" /></th>
						</s:if>
					</s:iterator>
				...
				<s:iterator value="page" status="status" id="pageRow">
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
								<s:if test="%{showDomain}">
		       						<td class="list"><a href='<s:url action="configmdm"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/>
		       							<s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property value="policyname" /></a></td>
		    					</s:if>
		    					<s:else>
									<td class="list"><a	href='<s:url action="configmdm"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'>
									<s:property value="policyname" /></a></td>
								</s:else>
							</s:if>
							<s:if test="%{columnId == 2}">
							...
							<s:if test="%{columnId == 3}">
								<td class="list">&nbsp;
								<s:if test="%{mdmType == 0}">Casper Suite</s:if>
								<s:if test='%{mdmType == 1}'>AirWatch</s:if>
								</td>
							</s:if>
							<s:if test="%{columnId == 4}">
								<td class="list">&nbsp;
								<s:property value="OsObject" />
								</td>
							</s:if>
						</s:iterator>
						<s:if test="%{showDomain}">
