...
<div id="content"><s:form action="ssidProfilesFull">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>"
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('ssidProfilesFull')"
						<s:property value="writeDisabled" />></td>
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						...
						<s:if test="%{columnId == 8}">
							<th><s:text name="config.ssid.column.cm" /></th>
						</s:if>
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
								<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="ssidProfilesFull.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'>
										<s:property value="ssidName" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="ssidProfilesFull.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="ssidName" /></a></td>
								</s:else>
							</s:if>
							<s:if test="%{columnId == 2}">
							...
							<s:if test="%{columnId == 8}">
								<td class="list" nowrap="nowrap">&nbsp;<s:property value="enabledCM" /></td>
							</s:if>
							<s:if test="%{columnId == 9}">
