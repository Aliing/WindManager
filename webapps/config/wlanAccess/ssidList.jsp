...
<div id="content"><s:form action="ssidProfiles">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>" 
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('ssidProfiles')"
						<s:property value="writeDisabled" />></td>
					</s:if>
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th><ah:sort name="ssidName" key="config.ssid.head.ssidName" /></th>
						</s:if>
						...
						<s:if test="%{columnId == 4 && !blnShowSsid}">
							<th><s:text name="config.ssid.userInfo.userCategory" /></th>
						</s:if>
						<s:if test="%{columnId == 5}">
							<th><s:text name="report.client.table.clientCWPUsed" /></th>
						</s:if>
						<s:if test="%{columnId == 6}">
							<th><ah:sort name="macAuthEnabled" key="config.ssid.enabledMAC" /></th>
						</s:if>
						<s:if test="%{columnId == 7}">
							<th><ah:sort name="maxClient" key="config.ssid.maxClient" /></th>
						</s:if>
						<s:if test="%{columnId == 8}">
							<th><ah:sort name="comment" key="config.ssid.description" /></th>
						</s:if>
						<s:if test="%{columnId == 9}">
							<th><ah:sort name="enabledIDM" key="config.ssid.column.idm" /></th>
						</s:if>
					</s:iterator>	
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
								<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="ssidProfiles.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'>
										<s:property value="ssidName" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="ssidProfiles.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="ssidName" /></a></td>
								</s:else>
							</s:if>
							<s:if test="%{columnId == 2}">
							...
							<s:if test="%{columnId == 4 && !blnShowSsid}">
								<td class="list" nowrap="nowrap"><s:property value="userCategoryString" /></td>
							</s:if>
							<s:if test="%{columnId == 5}">
								<td class="list" nowrap="nowrap"><s:property value="cwpString" /></td>
							</s:if>
							<s:if test="%{columnId == 6}">
								<td class="list"><s:property value="macAuthEnabled" /></td>
							</s:if>
							<s:if test="%{columnId == 7}">
								<td class="list"><s:property value="maxClient" /></td>
							</s:if>
							<s:if test="%{columnId == 8}">
								<td class="list" nowrap="nowrap">&nbsp;<s:property value="comment" /></td>
							</s:if>
							<s:if test="%{columnId == 9}">
								<td class="list" nowrap="nowrap">&nbsp;<s:property value="enabledIDM" /></td>
							</s:if>
						</s:iterator>
						<s:if test="%{showDomain}">
