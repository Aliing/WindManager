				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th><ah:sort name="ethernetName" key="config.ipFilter.name" /></th>
						</s:if>
						<s:elseif test="%{columnId == 2}">
							<th><ah:sort name="userProfile" key="config.ethernet.access.user.profile" /></th>
						</s:elseif>
						<s:elseif test="%{columnId == 3}">
							<th><ah:sort name="macLearning" key="config.ethernet.access.macLearn" /></th>
						</s:elseif>
						...
						<s:if test="%{showDomain && owner.domainName!='home' && owner.domainName!='global'}">
   						...
   						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
		   						<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="ethernetAccess.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="ethernetName" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="ethernetAccess.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="ethernetName" /></a></td>
								</s:else>
							</s:if>
							<s:elseif test="%{columnId == 2}">
							...
							<s:elseif test="%{columnId == 3}">
								<td class="list"><s:property value="stringMacLearn" /></td>
							</s:elseif>
							<s:elseif test="%{columnId == 4}">
