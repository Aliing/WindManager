...
<script>
var formName = 'cwpPageCustomization';
var thisOperation;
...
<div id="content">
<s:form action="cwpPageCustomization">
			...
			<td>
				<table id ="hiveTable" cellspacing="0" cellpadding="0" border="0" class="view">
					<tr>
						...
						<th><ah:sort name="name" key="config.cwp.page.customization.name" /></th>
						<th><ah:sort name="type" key="config.cwp.page.customization.type" /></th>
						<th><ah:sort name="description" key="config.cwp.page.customization.description" /></th>
					...
					<s:iterator value="page" status="status">
						...
						<tr class="<s:property value="%{#rowClass}"/>">
							...
							<s:if test="%{showDomain}">
								<td class="list"><a	href='<s:url value="cwpPageCustomization.action"><s:param name="operation" value="%{'edit'}"/>
									<s:param name="id" value="%{id}"/><s:param name="domainId" value="%{owner.id}"/></s:url>'><s:property
									value="name" /></a></td>
							</s:if>
							<s:else>
								<td class="list"><a	href='<s:url value="cwpPageCustomization.action"><s:param name="operation" value="%{'edit'}"/>
									<s:param name="id" value="%{id}"/></s:url>'><s:property	value="name" /></a></td>
							</s:else>
