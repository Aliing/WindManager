...
<div id="content"><s:form action="qosClassification">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>"
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('qosClassification')"
						<s:property value="writeDisabled" />></td>
					</s:if>
		...
		<tr>
			...
			<table cellspacing="0" cellpadding="0" border="0" class="view">
				<tr>
					...
					<th align="left" nowrap><ah:sort name="classificationName" key="config.qos.classification.name" /></th>
					</s:if>
					...
					<th align="left" nowrap><ah:sort name="prtclP" key="config.qos.protocolP" /></th>
					</s:if>
					<s:if test="%{columnId == 6}">
					<th align="left" nowrap><ah:sort name="prtclD" key="config.qos.protocolD" /></th>
					</s:if>
					<s:if test="%{columnId == 7}">
					<th align="left" nowrap><ah:sort name="prtclE" key="config.qos.protocolE" /></th>
					</s:if>
				...
				<s:iterator value="page" status="status" id="pageRow">
					...
					<s:if test="%{columnId == 1}">
					<s:if test="%{showDomain}">
       						<td class="list"><a href='<s:url action="qosClassification"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/>
       							<s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property value="classificationName" /></a></td>
    					</s:if>
    					<s:else>
							<td class="list"><a
								href='<s:url action="qosClassification"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
								value="classificationName" /></a></td>
						</s:else>
						</s:if>
						<s:if test="%{columnId == 2}">
						...
						<s:if test="%{columnId == 5}">
						<td class="list"><s:property value="chboxPValue" /></td>
						</s:if>
						<s:if test="%{columnId == 6}">
						<td class="list"><s:property value="chboxDValue" /></td>
						</s:if>
						<s:if test="%{columnId == 7}">
						<td class="list"><s:property value="chboxEValue" /></td>
						</s:if>
						<s:if test="%{columnId == 8}">
