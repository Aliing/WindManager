			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>"
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('qosRateControl')"
						<s:property value="writeDisabled" />></td>
		...
		<tr>
			...
			<table cellspacing="0" cellpadding="0" border="0"
				...
				<tr>
					...
					<th align="left" nowrap><ah:sort name="qosName"
						key="config.qos.classification.name" /></th>
					</s:if>
					<s:if test="%{columnId == 2}">
					<th align="left" nowrap><ah:sort name="rateLimit"
						key="config.qos.rateControl.rateLimit.abg" /></th>
					</s:if>
					<s:if test="%{columnId == 3}">
					<th align="left" nowrap><ah:sort name="rateLimit11n"
						key="config.qos.rateControl.rateLimit.11n" /></th>
					</s:if>
					<s:if test="%{columnId == 4}">
					<th align="left" nowrap><ah:sort name="rateLimit11ac"
						key="config.qos.rateControl.rateLimit.11ac" /></th>
					</s:if>
					<s:if test="%{columnId == 5}">
					<th align="left" nowrap><ah:sort name="description"
						key="config.qos.classification.description" /></th>
					</s:if>
					</s:iterator>
				...
				<s:iterator value="page" status="status" id="pageRow">
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:if test="%{columnId == 1}">
						<s:if test="%{showDomain}">
       						<td class="list"><a href='<s:url action="qosRateControl"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/>
       							<s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property value="qosName" /></a></td>
    					</s:if>
    					<s:else>
							<td class="list"><a
								href='<s:url action="qosRateControl"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
								value="qosName" /></a></td>
						</s:else>
						</s:if>
						<s:if test="%{columnId == 2}">
						<td class="list"><s:property value="rateLimit" /></td>
						</s:if>
						<s:if test="%{columnId == 3}">
						<td class="list"><s:property value="rateLimit11n" /></td>
						</s:if>
						<s:if test="%{columnId == 4}">
						<td class="list"><s:property value="rateLimit11ac" />&nbsp;</td>
						</s:if>
						<s:if test="%{columnId == 5}">
