...
<div id="content"><s:form action="hiveProfiles">
			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>"
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('hiveProfiles')"
						<s:property value="writeDisabled" />></td>
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th><ah:sort name="hiveName" key="config.hp.name" /></th>
						</s:if>
						<s:if test="%{columnId == 2}">
							<th><ah:sort name="l3TrafficPort" key="config.hp.l3TrafficPort" /></th>
						</s:if>
						<s:if test="%{columnId == 3}">
							<th><ah:sort name="enabledPassword" key="config.hp.enabledPassword" /></th>
						</s:if>
						<s:if test="%{columnId == 4}">
							<th><ah:sort name="rtsThreshold" key="config.hp.rtsThreshold" /></th>
						</s:if>
						<s:if test="%{columnId == 5}">
							<th><ah:sort name="fragThreshold" key="config.hp.fragThreshold" /></th>
						</s:if>
						<s:if test="%{columnId == 6}">
							<th><ah:sort name="description" key="config.hp.description" /></th>
						</s:if>
					</s:iterator>
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
								<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="hiveProfiles.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="hiveName" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="hiveProfiles.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="hiveName" /></a></td>
								</s:else>
							</s:if>
							<s:if test="%{columnId == 2}">
								<td class="list"><s:property value="l3TrafficPort" /></td>
							</s:if>
							<s:if test="%{columnId == 3}">
								<td class="list"><s:property value="displayEnabledPassword" /></td>
							</s:if>
							<s:if test="%{columnId == 4}">
								<td class="list"><s:property value="rtsThreshold" /></td>
							</s:if>
							<s:if test="%{columnId == 5}">
								<td class="list"><s:property value="fragThreshold" /></td>
							</s:if>
							<s:if test="%{columnId == 6}">
