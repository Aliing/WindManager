			...
			<table border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td><input type="button" name="ignore" value="<s:text name="button.paintbrush"/>"
						id="brushTriggerBtn"
						class="button" onclick="hm.paintbrush.triggerPaintbrush('tunnelSetting')"
						<s:property value="writeDisabled" />></td>
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						<s:if test="%{columnId == 1}">
							<th><ah:sort name="tunnelName" key="config.tunnelSetting.name" /></th>
						</s:if>
						<s:if test="%{columnId == 2}">
							<th><ah:sort name="enableType" key="config.tunnelSetting.enableType" /></th>
						</s:if>
						<s:if test="%{columnId == 3}">
							<th><ah:sort name="description" key="config.tunnelSetting.description" /></th>
						</s:if>
					</s:iterator>
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
								<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="tunnelSetting.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="tunnelName" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="tunnelSetting.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="tunnelName" /></a></td>
								</s:else>
							</s:if>
							<s:if test="%{columnId == 2}">
								<td class="list"><s:property value="enableTypeString" /></td>
							</s:if>
							<s:if test="%{columnId == 3}">
