...
var filterFormName = 'tvStudentFilter';
...
function initialValues(){
	document.getElementById(filterFormName+"_filterStudentId").value = '';
	document.getElementById(filterFormName+"_filterStudentName").value = '';
	document.getElementById(filterFormName+"_filterClassId").value = '-1';
}
				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						...
						<s:elseif test="%{columnId == 1}">
							<th align="left" nowrap><ah:sort name="tvClass.className"
						...
						<s:if test="%{showDomain && owner.domainName!='home' && owner.domainName!='global'}">
   						...
   						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 4}">
		   						<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="tvStudentRoster.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="studentId" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="tvStudentRoster.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="studentId" /></a></td>
								</s:else>
							</s:if>
							<s:elseif test="%{columnId == 1}">
								<td class="list"><s:property value="tvClass.className" />&nbsp;</td>
							</s:elseif>
							<s:elseif test="%{columnId == 3}">
								<td class="list"><s:property value="description" />&nbsp;</td>
							</s:elseif>
							<s:elseif test="%{columnId == 2}">
								<td class="list"><s:property value="studentName" />&nbsp;</td>
							</s:elseif>
						</s:iterator>
						<s:if test="%{showDomain}">
...
<div id="filterPanel" style="display: none;">
	...
	<div class="bd">
		<s:form action="tvStudentRoster" id="tvStudentFilter"
			name="tvStudentFilter">
							...
							<tr>
								<td style="padding: 6px 5px 5px 5px;">
									<table cellspacing="0" cellpadding="0" border="0" width="100%">
										<tr>
											<td class="labelT1">
												<s:text name="config.tv.studentId" />
											</td>
											<td>
												<s:textfield name="filterStudentId" maxlength="31"
													size="30" />
											</td>
										</tr>
										<tr>
											<td class="labelT1">
												<s:text name="config.tv.studentName" />
											</td>
											<td>
												<s:textfield name="filterStudentName" maxlength="128"
													size="30" />
											</td>
										</tr>
										<tr>
											<td class="labelT1">
												<s:text name="config.tv.class" />
											</td>
											<td>
												<s:select
													name="filterClassId" list="%{filterClassList}" listKey="id"
													listValue="value" cssStyle="width: 175px;" headerKey="-1" headerValue=""/>
											</td>
										</tr>
									</table>
								</td>
