				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						...
						<s:if test="%{columnId == 2}">
							<th><ah:sort name="reportType" key="report.reportList.title.type" /></th>
						</s:if>
						<s:if test="%{columnId == 3}">
							<th><ah:sort name="reportDetailType" key="report.reportList.excuteType" /></th>
						</s:if>
						<s:if test="%{columnId == 4}">
							<th><ah:sort name="sortByType" key="report.customReport.orderType" /></th>
						</s:if>
						<s:if test="%{columnId == 5}">	
							<th><ah:sort name="longSortBy" key="report.customReport.sortingKey" /></th>
						</s:if>
						<s:if test="%{columnId == 6}">	
							<th><ah:sort name="description" key="report.customReport.description" /></th>
						</s:if>
					</s:iterator>
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
								<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="customReportList.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="name" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="customReportList.action"><s:param name="operation" value="%{'edit'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="name" /></a></td>
								</s:else>
							</s:if>
							<s:if test="%{columnId == 2}">
								<td class="list"><s:property value="reportTypeString" /></td>
							</s:if>
							<s:if test="%{columnId == 3}">
								<td class="list"><s:property value="reportDetailTypeString" /></td>
							</s:if>
							<s:if test="%{columnId == 4}">
								<td class="list">&nbsp;<s:property value="sortByTypeString" /></td>
							</s:if>
							<s:if test="%{columnId == 5}">
								<td class="list">&nbsp;<s:property value="longSortByString" /></td>
   							</s:if>
							<s:if test="%{columnId == 6}">
