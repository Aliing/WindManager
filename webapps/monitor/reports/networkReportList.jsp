				...
				<tr>
					...
					<s:iterator value="%{selectedColumns}">
						...
						<s:if test="%{columnId == 3}">
							<th><ah:sort name="excuteType" key="report.reportList.title.type" /></th>
						</s:if>
						<s:if test="%{columnId == 4}">
							<th><s:text name="report.reportList.reportPeriod" /></th>
						</s:if>
						<s:if test="%{columnId == 5}">
							<th><ah:sort name="description" key="report.reportList.clientAuth.description" /></th>
						</s:if>
					</s:iterator>
					...
					<tr class="<s:property value="%{#rowClass}"/>">
						...
						<s:iterator value="%{selectedColumns}">
							<s:if test="%{columnId == 1}">
								<s:if test="%{showDomain}">
									<td class="list"><a
										href='<s:url value="networkUsageReport.action"><s:param name="operation" value="%{'runNow'}"/><s:param name="id" value="%{#pageRow.id}"/><s:param name="domainId" value="%{#pageRow.owner.id}"/></s:url>'><s:property
										value="name" /></a></td>
								</s:if>
								<s:else>
									<td class="list"><a
										href='<s:url value="networkUsageReport.action"><s:param name="operation" value="%{'runNow'}"/><s:param name="id" value="%{#pageRow.id}"/></s:url>'><s:property
										value="name" /></a></td>
								</s:else>
							</s:if>
							<s:if test="%{columnId == 2}">
							...
							<s:if test="%{columnId == 4}">
								<td class="list">&nbsp;<s:property value="reportPeriodString" /></td>
							</s:if>
							<s:if test="%{columnId == 5}">
