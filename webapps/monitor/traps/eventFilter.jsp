...
<script>
var filterFormName = 'eventFilter';
...
function filterChanged(value,changeype){
	...
	if(value == -1){
		submitFilterAction('resetFilter');
	}else{
		url = "<s:url action='events' includeParams='none' />" + "?operation=requestFilterValues&filter=" + encodeURIComponent(value) + "&ignore=" + new Date().getTime();
...
var filterResult = function(o) {
	...
	if(result.eComponent){
		document.getElementById("component").value = result.eComponent;
	}else{
		document.getElementById("component").value = "";
	}
	if(result.eBeginDate){
		document.getElementById("beginDate").value = result.eBeginDate;
	}else{
		document.getElementById("beginDate").value = "";
	}
	if(result.eBeginTime){
		document.getElementById("beginTimeH").value = parseInt(result.eBeginTime);
	}else{
		document.getElementById("beginTimeH").value = 0;
	}
	if(result.eEndDate){
		document.getElementById("endDate").value = result.eEndDate;
	}else{
		document.getElementById("endDate").value = "";
	}
	if(result.eEndTime){
		document.getElementById("endTimeH").value = parseInt(result.eEndTime);
	}else{
...
<div id="leftFilter">
	...
	<div id="filterPanel" style="display: none;">
		...
		<div class="bd">
			<s:form action="events" id="eventFilter" name="eventFilter">
